 ...
 */
export function initPointerLayout(text, measure, mode, spacing = 1) {
    const chars = [...text];
    const glyphs = [];
    let cursor = 0;
 ...
 */
export function stepPointerLayout(state, pointer, radius, strength, dt = 1 / 60) {
    ...
    const glyphs = state.glyphs.map((glyph) => {
        ...
        const distToTarget = Math.sqrt((glyph.position.x - targetX) ** 2 + (glyph.position.y - targetY) ** 2);
...
export function isAtRest(state) {
    return state.glyphs.every((g) => {
        ...
        const distToBase = Math.sqrt((g.position.x - g.base.x) ** 2 + (g.position.y - g.base.y) ** 2);
